...
def clean_csv(csv_file):
    ...
    df['duration_of_use'] = df['duration_of_use'].astype(int)
    df['motion_sickness'] = df['motion_sickness'].astype(int)
    df['immersion_level'] = df['immersion_level'].astype(int)
    return df.to_csv('vr_experience_cleaned.csv', encoding='utf-8', index=False)
...
def convert_to_json(csv_file, jsonFilePath):
    ...
    with open(csv_file, encoding='utf-8') as csvf:
        ...
        for rows in csvReader:
            ...
            rows['duration_of_use'] = int(rows['duration_of_use'])
